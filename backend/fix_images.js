...
const gayaPath = path.join('..', 'frontend', 'lib', 'data', 'gayaPlaces.js');
const nearbyPath = path.join('..', 'frontend', 'lib', 'data', 'nearbyPlaces.js');

let gayaContent = fs.readFileSync(gayaPath, 'utf8');
let nearbyContent = fs.readFileSync(nearbyPath, 'utf8');

// We will use a reliable Unsplash collection for fallbacks if Wiki fails
const fallbacks = {
...
async function processFile(content, outPath) {
  // We'll use regex to find id, title, link, category
  // But a safer way is to regex replace the image: "..." line.
  
  // To know the title/link for a block, we can split by '{' and '}'
  ...
  for (let block of blocks) {
    ...
    if (titleMatch) {
      ...
      if (!imageUrl) {
        // Try searching wikipedia for the exact title
        imageUrl = await getWikiImage(title.split(' ')[0]); // try first word
      }
      ...
      if (!imageUrl) {
        ...
        console.log(`  -> Found Wiki image for ${title}`);
      }
...
async function main() {
  await processFile(gayaContent, gayaPath);
  await processFile(nearbyContent, nearbyPath);
}
